...
def sentence_core_rank_score(nbest_compressions):
    ll = len(nbest_compressions)
    scores = np.zeros(ll)
    for i in len(ll):
        sentence = nbest_compressions[i][1]
        for j in len(sentence):
            ## stop word score, remove or not??
            scores[i] += core_rank_score[sentence[j][0]]
    return scores

def fluency_score(nbest_compressions):
    ll = len(nbest_compressions)
    lan_model = word2vec.Word2Vec.load('text8.model')
    
def final_score(nbest_compressions):
    ll = len(nbest_compressions)
    scores = np.zeros(ll)
    cr_score = sentence_core_rank_score(nbest_compressions)
    fl_score = fluency_score(nbest_compressions)
    for i in len(ll):
        sentence_len = len(nbest_compressions[i][1])
        scores[i] = nbest_compressions[i][0]*fl_score[i]/(sentence_len*cr_score[i])
    return scores
