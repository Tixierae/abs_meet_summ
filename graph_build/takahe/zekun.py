...
class word_graph:
    ...
    def build_graph(self):
        ...
        for i in range(self.length):
            ...
            for j in range(sentence_len):
                ...
                syn_candidates = self.synonyme_nodes(node)
                hyp_candidates = self.hypernyme_nodes(node)
            ...
            #-------------------------------------------------------------------
            # 5. Connects the mapped words with directed edges
            #-------------------------------------------------------------------
            print mapping
    ...
    def tagging (self, tag):
        if tag.startswith('V'):
            ...
            return ''
    #**************************************************************************
    # check for synonyme, hypernyme, entailment nodes
    #**************************************************************************
    ...
    def hypernym_nodes(self,node):
        ...
        if pos == '':
            return hyp_nodes
        for syn in wn.synsets(ref_word, pos=ref_pos):
    ...
    def common_hypernym_nodes(self, node):
        hyp_nodes = []  # return [[node1, common_hyp1],[node2, common_hyp2],...]
        hyps = []       # hypernymes of existing node 
        hyp_word = []   # hypernymes of this word
        word, tag = node.split(self.sep)
        pos = self.tagging(tag) 
        if pos == '':
            return hyp_nodes, ''
        ...
        for gnode in self.graph.nodes():
            ...
            for syn in wn.synsets(ref_word, pos=ref_pos):
                for hyp in syn.hypernyms():
                    hyp_candidate = hyp.name().split('.')[0]
                    if (hyp in hyp_word) and ([gnode, hyp_candidate] not in hyp_nodes):
                        hyp_nodes.append([gnode, hyp_candidate])
        return hyp_nodes 
