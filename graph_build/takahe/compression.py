    ...
    def build_graph(self):
        ...
        for i in range(self.length):
            ...
            for j in range(sentence_len):
                ...
                sec_candidates = self.candidates(node)
                c = len(sec_candidates)
                ...
                for ki in range(k):
                    for sid, pos_s in self.graph.node[(node, ki)]['info']:
                            if (sid != i) and ((node, ki) not in first_candidates):
                                first_candidates.append((node, ki))
                k_net = len(first_candidates)

                for sec_candidate in sec_candidates:
                    for sid, pos_s in self.graph.node[sec_candidate]['info']:
                        if (sid == i):
                            sec_candidates.remove(sec_candidate)
                ...
                if (k_net == 0) and (c_net==0):
                    ...
                    mapping[j] = sec_candidates[result]
                else:
                    result = self.best_candidate(first_candidates,i,j)
                    self.graph.node[first_candidates[result]]['info'].append((i,j))
